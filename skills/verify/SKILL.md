...
- **Called by execute** (automatic): runs as `claude -p` subprocess — completely separate OS process, zero shared context with builder. Execute writes prompt to temp file, runs `claude -p`, parses stdout.
