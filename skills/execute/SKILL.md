...
2. **Check for resume**: If `.claude/ralph-loop.local.md` exists AND references this spec:
...
4. **Validate format**: Run `verify-spec.py` from clarify skill if available:
   - Search for script: Glob `**/clarify/scripts/verify-spec.py` in `~/.claude/skills/` and project root
...
3. **Auto-activate ralph-loop** if spec has >1 stage OR >5 tasks:
   ```bash
   mkdir -p .claude
   ...
   ---
   Read <spec-path>. Find first stage with **Status**: todo tasks.
   Execute it (3a→3b→3c). Update task statuses in spec.
   If all tasks verified — <promise>ALL DONE</promise>.
   EOF
   ```
   For small specs (1 stage, ≤5 tasks): skip ralph-loop.
...
FOR each stage in Execution Order:
  3a. Plan stage (batch-style research + decomposition, NO native plan mode)
  3b. Execute stage (spawn [P] workers + serial tasks)
  3c. Verify stage (independent claude -p subprocess)
...
This pattern (from autoresearch) ensures correctness after `/compact` or ralph-loop re-injection.

#### 3a. Plan Stage (batch-style, NO EnterPlanMode)

**Do NOT call EnterPlanMode** — spec is already approved. This is execution-time planning.

1. **Research** codebase for this stage's tasks:
   - Launch Explore subagent(s) in foreground to scan files referenced in Leverage and Files fields
...
Spawn ALL [P] tasks in this stage as worktree agents in **one message block**:

```
Agent(
  description="TASK-{N}: {title}",
  isolation="worktree",
...
Note: workers only need [Bash, Glob, Grep, Read, Edit, Write]. The worker prompt explicitly scopes their work to listed Files.
```
...
- **DONE** → merge worktree branch:
  ...
  ```
  - If merge succeeds → update spec on disk: find `### TASK-{N}:` section, change `**Status**: todo` → `**Status**: done`. Use Edit tool (not memory — read the spec file, find the exact line, replace).
  - If merge conflicts:
...
Verify runs as a **separate Claude Code process** — zero shared context with execute/builder.

**Pre-check**: Verify `claude` CLI is available:
```bash
command -v claude >/dev/null 2>&1
```
If not available → fall back to in-process verification (run proof commands directly in current session). Log concern: "claude CLI not found — verification ran in-process (shared context, independence not guaranteed)".
...
2. Run as independent process:
   ```bash
   claude -p "$(cat /tmp/verify-prompt.txt)" --allowedTools "Bash,Read,Glob,Grep" > /tmp/verify-result.txt 2>&1
   ```
...
**Why subprocess**: `claude -p` creates a completely separate session. The verifier:
...
4. **Optional: cross-model review** (if codex plugin installed):
   After AC verify passes, run Codex adversarial review for code quality:
   ```bash
   # Check if openai/codex-plugin-cc installed
   ls ~/.claude/plugins/codex-plugin-cc 2>/dev/null && HAS_CODEX=true
   ```
   If available, suggest: "AC verification passed. Run `/codex:adversarial-review` for cross-model code review? (Codex challenges design decisions, tests assumptions)"
   
   Plugin: [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc)
   Install: `/install-plugin openai/codex-plugin-cc`
   
   Codex review is SUPPLEMENTARY — it checks code quality/design, not AC compliance.
   AC verify (claude -p) remains the primary gate.
...
- Deactivate ralph-loop: `rm .claude/ralph-loop.local.md` (if exists)
...
Auto-activated in Phase 2 for specs with >1 stage or >5 tasks.
Deactivated on: all stages verified, or stage failure after max fix iterations.
State persistence: spec file `**Status**` fields are the source of truth — ralph reads spec to find next `todo` stage.
