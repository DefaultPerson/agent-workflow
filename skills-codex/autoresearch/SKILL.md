...
description: >
  ...
  failures. Uses codex resume for non-stop execution across context limits.
  Triggers: "autoresearch", "/autoresearch", "auto research", "optimize metric",
  "автоисследование", "оптимизируй метрику"
allowed-tools: [shell, glob_file_search, rg, read_file, apply_patch]
---
...
Extract the optimization goal from `$ARGUMENTS`. If empty — ask what to optimize.
...
### 0.7 Enable auto-resume

For non-stop execution across context limits, use Codex built-in resume:

```bash
codex resume --last "Continue autoresearch. Read autoresearch-scratchpad.md for full context and current state."
```
...
When done (after completion or manual cancel):
