...
allowed-tools: [shell, glob_file_search, rg, read_file, apply_patch, agent, web_search, web_fetch]
---
...
4. **Scan codebase** (if project directory exists):
   ...
   - Use glob_file_search/rg to understand the current state.
...
Output the question as text and wait for user response:
```
